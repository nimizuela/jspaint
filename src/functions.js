...
function reset_canvas(){
	...
	bg_canvas.width = my_canvas_width;
	bg_canvas.height = my_canvas_height;
	...
	if(!not_resizable_canvas && (img.width > canvas.width || img.height > canvas.height)){
		...
		$w.$Button("Enlarge", function(){
			...
			undoable(function(){
				...
				bg_canvas.width = Math.max(original.width, img.width);
				bg_canvas.height = Math.max(original.height, img.height);
				ctx.disable_image_smoothing();
				if(!transparency){
					ctx.fillStyle = colors.background;
					ctx.fillRect(0, 0, canvas.width, canvas.height);
